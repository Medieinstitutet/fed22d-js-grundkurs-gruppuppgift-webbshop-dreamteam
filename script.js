...
function printOrdredDonuts() {
	...
	for(let i = 0; i < donuts.length; i++) {
		if(donuts[i].amount > 0 && donuts[i].amount <= 9) {
			...
			document.querySelector(".cartName").innerHTML += ` <span class="cartNameProduct">Produkt: ${
        donuts[i].name
		...
		if(newDate.getDay() === 1 && newDate.getHours() < 10) {
			...
			(newDate.getDay() === 5 && newDate.getHours() > 15) || newDate.getDay() === 6 || newDate.getDay() === 0 || (newDate.getDay() === 1 && newDate.getHours() < 3)) {
			donutPriceTotal = Math.round(donutPriceRegular * 1.15);
		} else if(weekNum % 2 === 0 && newDate.getDay() === 2 && donutPriceRegular > 25) {
	...
	if(donutTotalAmount < 16 && donutTotalAmount >= 1) {
		...
		document.querySelector(".shippingValue").innerHTML = Math.round(shippingTotal);
...
if(newDate.getMonth() === 11 && newDate.getDate() === 13) {
	let luciaDonut = 1;
	document.querySelector(".cartName").innerHTML += `<div> <span class="cartNameProduct">Produkt: Luciamunk <br></span> <span class="cartNameQuantity">Antal: 1</span> <span class="cartNamePrice">Pris: 0<br><hr></span></div>`; // ÄNDRA
}
///////////////// Jultema//////////////////////
if(newDate.getMonth() === 11 && newDate.getDate() === 24) {
	...
	document.body.style.backgroundImage = "linear-gradient(45deg, #9e001d 25%, #ffffff 25%, #ffffff 50%, #9e001d 50%, #9e001d 75%, #ffffff 75%, #ffffff 100%)";
...
function activatesubmitButton() {
	if(checkfNameInput && checklNameInput && checkemailInput && checkphoneInput && checkstreetInput && checkzipInput && checkcityInput && checkGdprInput && ((checkcardNrInput && checkdateNrInput && checkcvcNrInput && radiocard.checked) || (checkpersonNrInput && radioinvoice.checked))) {
